...
- **One line per thirty seconds, per pass, and the opener is exempt.** A pass that finishes inside the
  floor costs exactly two lines. The floor is per pass rather than per run, because a shared floor means
  five processes coordinating through a file for nothing.
