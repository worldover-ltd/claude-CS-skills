...
A copy of `skills/upload-documents/lib/progress.py`, which is where a change is made first. It is
duplicated rather than imported because `extract_documents.py` runs under `uv` in an interpreter of its
own, which reaches nothing outside this directory.
"""
