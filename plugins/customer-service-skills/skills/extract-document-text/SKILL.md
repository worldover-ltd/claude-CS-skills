...
It reports while it runs, on stderr: one line as a pass starts, one thirty seconds in carrying an
estimate, one five minutes later, one every fifteen minutes after that, and one when it ends. The summary follows on stdout — the count per `kind`, and the file names
...
measured. `docs/PRESENTING.md` in this plugin holds the vocabulary. A pass finishing inside the first
thirty seconds costs two lines, and one that dies says so in one.
